...
if [ -f "$DATA_DIR/HG00514.fastq.gz" ]; then
    echo "=== HG00514.fastq.gz ya existe, saltando descarga ==="
else
    ...
    echo "=== Verificando FASTQ ==="
    N_READS=$(zcat HG00514.fastq.gz | head -400 | grep "^@" | wc -l)
    echo "Reads en primeras 400 líneas: $N_READS"
    if [ "$N_READS" -eq 0 ]; then
        echo "ERROR: FASTQ vacío o corrupto, abortando"
        exit 1
    fi
fi

FASTQ_SIZE=$(du -sh $DATA_DIR/HG00514.fastq.gz | cut -f1)
echo "Tamaño FASTQ: $FASTQ_SIZE"
echo "=== FASTQ listo: $(ls -lh $DATA_DIR/HG00514.fastq.gz) ==="
