---
title: "TE analysis report T2T-CHM13v2.0"
...
chrsLength = c(248387328, 242696752, 201105948, 193574945, 182045439, 172126628,
               160567428, 146259331, 150617247, 134758134, 135127769, 133324548,
               113566686, 101161492, 99753195, 96330374, 84276897, 80542538, 61707364,
...
```{r data}
allIns = readRDS(params$all_ins)
insertionsTable = readRDS(params$insertions_table)
...
- `r allIns[repeat.class %in% c(class1, class2) & repeat.percentage >= 0.85 & SUPP_trusty > 0] %>% nrow()` TE insertions.
...
strictWithoutAlleles = strictWithoutAllelesMask %>%
...
- `r nrow(strictWithFalsePositives)` unique insertions that passed the stringent filtering (`r nrow(strictWithFalsePositives) / nrow(insertionsTable[SUPP_trusty >= 1]) * 100`% of the total unique insertions), had at least one occurrence genotyped as 0/0.
...
- `r w` of these unique insertions were genotyped as 0/0 for all their occurrences, amounting to `r W` insertion counts.
- Of these fully wrong calls, `r w_tick` were only found on a single patient each (`r w_tick / w * 100`% of the `r w`).
...
```{r table4, echo = F}
  ...
  min3WithoutAlleles = min3WithoutAllelesMask %>%
     lapply(as.numeric) %>% lapply(sum)
  ...
  allelesWithoutMin3 = allelesWithoutMin3Mask %>%
     lapply(as.numeric) %>% lapply(sum)
...
copy = copy(insertionsTable[SUPP_trusty > 0])[, `:=`(seqnames = seqnames %>% sub(pattern = "chr(.+)", replacement = "\\1", x = .) %>% as.numeric())]
copy = copy[!is.na(seqnames)]
qqman::manhattan(
  copy,
  chr = "seqnames",
  bp = "start",
...
```{r}
roi = annotatedInsertionsMin3[repeat.class %in% c(class1, class2) & SUPP_trusty > 0 & repeat.percentage >= 0.85]
...
```{r}
meDeletionsMin3 = readRDS(params$me_deletions)
```
...
- These deletions have a mean prevalence per sample of `r meDeletionsMin3[SUPP_trusty > 0]$SUPP_trusty %>% mean()` ± `r meDeletionsMin3[SUPP_trusty > 0]$SUPP_trusty %>% sd()` (median of `r meDeletionsMin3[SUPP_trusty > 0]$SUPP_trusty %>% median()`).


```{r}
meDeletionsMin3[SUPP_trusty > 0,
  .(
    Unique = .N,
    `Total\n(lax)` = sum(SUPP_min3),
    `Total\n(strict)` = sum(SUPP_trusty)
...
copy = copy(meDeletionsMin3[SUPP_trusty > 0])[, `:=`(
    seqnames = factor(seqnames, levels = chrs) %>% as.integer()
  )]
qqman::manhattan(
  copy,
  chr = "seqnames", bp = "start", snp = "id", p = "maf",
  logp = F, cex = 0.5, cex.axis = 0.9, ylab = "AF",
  chrlabs = chrs %>% sub(pattern = "chr(.+)", replacement = "\\1", x = .)
)
```
```{r fig.align="center",fig.cap="Allele frequency is shown for TE deletions for each chromosome. Each dot marks a different deletion"}
copy = copy(meDeletionsMin3[SUPP_trusty > 0])[, `:=`(
...
```{r fig.dim=c(14, 8), fig.align="center", fig.cap="Unique TE deletions occurrences on the genome by chromosome."}
meDeletionsMin3[seqnames %in% chrs &
          SUPP_trusty > 0, .SD[1],
        by = c("id")][, seqnames := factor(seqnames, levels = chrs)] %>%
  ggplot(data = ., aes(x = start, fill = seqnames)) +
  geom_histogram(bins = 35) +
  theme_light() +
  theme(
...
```{r}
del_roi = meDeletionsMin3[repeat.class %in% c(class1, class2) & SUPP_trusty > 0]
del_ioi = del_roi[, .SD[1], by = id][, c("id", "seqnames", "start", "end", "repeat.class", "repeat.subclass",
                                         "name", "svlen", "SUPP_min3", "SUPP_trusty")]
```
...
<details><summary>Click to show table with TE deletion counts by name.</summary>
```{r}
del_ioi[SUPP_trusty > 0, .(
    Name = gsub("_", " ", name),
    Unique = .N,
    `Total\n(lax)` = sum(SUPP_min3),
    `Total\n(strict)` = sum(SUPP_trusty),
    `Mean\nSVLEN` = mean(svlen) %>% as.integer()
  ), by = .(name)][,-"name"][order(-Unique)] %>%
  flextable::flextable() %>%
  flextable::theme_booktabs() %>%
  flextable::border_inner_h(border = flextable::fp_border_default()) %>%
  flextable::align(align = "center", part = "all") %>%
...
```{r results="asis"}
del_alleles = del_ioi[SUPP_trusty > 0]
if (nrow(del_alleles) == 0) {
  cat("No TE deletions were found.")
} else {
  cat(paste0("<details><summary>Click to show table with ", nrow(del_alleles)," deletion entries.</summary>\n\n"))
  del_alleles[
      , .(Position = paste0(seqnames, ":", start %>% format(big.mark = "\u202F", trim = T)),
          Length = svlen,
          Type = paste0(repeat.class, " (", repeat.subclass, ")"),
          Name = gsub("_", " ", name),
          Occurrences_lax = SUPP_min3,
          Occurrences_strict = SUPP_trusty
      )
      ][stringi::stri_order(Position, numeric = T)] %>%
  flextable::flextable() %>%
  flextable::autofit() %>%
...
# Retrotransposition hallmarks

```{r hallmarks_data}
if (file.exists(params$hallmarks) && file.info(params$hallmarks)$size > 0) {
  hallmarks = read.table(params$hallmarks, header = TRUE, sep = "\t", stringsAsFactors = FALSE)
  hallmarks$has_polya    = as.logical(hallmarks$has_polya)
  hallmarks$has_l1_motif = as.logical(hallmarks$has_l1_motif)
} else {
  hallmarks = data.frame(seqId = character(), has_polya = logical(), has_l1_motif = logical())
}
```

The search for retrotransposition hallmarks was performed on `r nrow(hallmarks)` strict TE insertions.

- `r sum(hallmarks$has_polya)` insertions (`r round(sum(hallmarks$has_polya) / nrow(hallmarks) * 100, 1)`%) present a polyA tail.
- `r sum(hallmarks$has_l1_motif)` insertions (`r round(sum(hallmarks$has_l1_motif) / nrow(hallmarks) * 100, 1)`%) present the L1 endonuclease target motif.
- `r sum(hallmarks$has_polya & hallmarks$has_l1_motif)` insertions (`r round(sum(hallmarks$has_polya & hallmarks$has_l1_motif) / nrow(hallmarks) * 100, 1)`%) present both hallmarks.

## Summary by TE family

```{r hallmarks_summary}
hallmarks_ioi = merge(
  hallmarks,
  ioi[, .(seqId, repeat.class, repeat.subclass)],
  by = "seqId",
  all.x = TRUE
)
setDT(hallmarks_ioi)

hallmarks_summary = hallmarks_ioi[
  repeat.class %in% c("Retroposon", "SINE", "LINE"),
  .(
    Total          = .N,
    `polyA`        = sum(has_polya),
    `L1 motif`     = sum(has_l1_motif),
    `Both`         = sum(has_polya & has_l1_motif)
  ),
  by = .(repeat.class, repeat.subclass)
][order(repeat.class, repeat.subclass)] %T>%
  setnames(c("repeat.class", "repeat.subclass"), c("Family", "Subfamily"))
...
```{r hallmarks_detail, results="asis"}
hallmarks_detail = hallmarks_ioi[has_polya == TRUE | has_l1_motif == TRUE]
if (nrow(hallmarks_detail) == 0) {
  cat("No insertions with retrotransposition hallmarks were found.")
} else {
  cat(paste0("<details><summary>Click to show table with ", nrow(hallmarks_detail), " insertions with at least one hallmark.</summary>\n\n"))
  hallmarks_detail[
    , .(
        seqId     = seqId,
        Family    = paste0(repeat.class, " (", repeat.subclass, ")"),
        `polyA`   = fifelse(has_polya,    "\u2713", "\u2717"),
        `L1 motif`= fifelse(has_l1_motif, "\u2713", "\u2717")
      )
  ] %>%
  flextable::flextable() %>%
  flextable::autofit() %>%
...
library(enrichplot)
library(DOSE)
pLimit = params$plimit %>% as.numeric()
```
...
fullNames = list(MF = "Molecular Function", BP = "Biological Process", CC = "Cellular Component")
for (x in c("MF", "BP", "CC")) {
  name = paste0("ego", x)
  dtName = paste0(name, "Dt")
  paramKey = name
  assign(x = name, readRDS(params[[paramKey]]))
  object = get(name)
  ...
  object@result %>% data.table() %>%
    .[, `:=`(
      "geneID" = NULL,
      GeneRatioCalc = GeneRatio %>% lapply(function (x) str2expression(x) %>% eval()) %>% unlist()
    )] %>% assign(dtName, ., envir = .GlobalEnv)
  dt = get(dtName)[p.adjust <= pLimit]
  if (nrow(dt) == 0) {
    cat("No significant results were found for this group.\n\n")
    next()
  }
  object@result = object@result[object@result$p.adjust <= pLimit, ]
  barplot(object, showCategory = 12, font.size = 10, title = paste0("GO - ", x)) %>% print()
  topN = min(nrow(dt), 10)
  cat("\n\nThe category with lowest p value is *", dt[order(p.adjust)][1, Description],
      "* (", dt[order(p.adjust)][1, p.adjust], ")", sep = "")
  if (nrow(dt > 1))
    cat(", and the highest (among the lowest ", topN,") is *",
      dt[order(p.adjust)][topN, Description] %>% as.character(), "* (",
      dt[order(p.adjust)][topN, p.adjust],")", sep = '')
  cat('.\n\n')
  n = min(100, nrow(dt))
  cat(paste0("<details><summary>Click to show/hide table with ", n, " rows.</summary>\n\n"))
  ...
  cat("</details>\n\n")
  emapplot(get(name) %>% pairwise_termsim(), showCategory = 30,
    cex.params = list(category_label = 0.5)) %>% print()
  cnetplot(get(name), showCategory = 30,
    cex.params = list(category_label = 0.5, gene_label = 0.5),
    node_label = "gene") %>% print()
}
```
...
```
<!-- Sets comparison disabled in Nextflow version (no otherSets input) -->
<!-- ```{r eval=F,child="sets.Rmd"} -->
<!-- ``` -->
